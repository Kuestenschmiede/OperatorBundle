...
function reactRenderReady() {
    ...
    if (!window.reactRenderReadyDone) {
        ...
        const jQueryinputSearch = jQuery('.form-view__searchinput > input');
        const jQuerydatepickerInput = jQuery(".react-datepicker__input-container");
        const jQuerydatepickerPopper = jQuery(".react-datepicker-popper");
        const jQueryinputPriceRadio = jQuery(".form-check-input[type=radio]");

        jQueryinputSearch.on('keydown focusout click', findAllFilterState);
        // jQueryinputSearch.on('focusout', findAllFilterState);
        jQuerydatepickerInput.on('focusin focusout click', findAllFilterState);
        jQueryinputPriceRadio.on('click', setFilterButtonActive);

        // set label text as placeholder for input
        const jQueryofferLabel = jQuery(".offer-filter__searchinput label");
        const jQueryofferLabelText = jQuery(".offer-filter__searchinput label").text();
        jQueryofferLabel.hide();
        const jQueryofferInputField = jQuery(".offer-filter__searchinput input");

        jQueryofferInputField.attr("placeholder", jQueryofferLabelText);
...
function findAllFilterState() {
    ...
    // check search input
    const jQueryinputSearch = jQuery('.form-view__searchinput > input');
    if (jQueryinputSearch.val()) {
        searchInput = true;
    }

    // check filter tags
    const jQuerytagFilterItemLabel = jQuery(".tag-filter__filter-item > label");
    if (jQuerytagFilterItemLabel.hasClass("checked-tag-filter")) {
        filterTag = true;
    }

    // check datepicker input value
    const jQuerydatepickerInput = jQuery(".react-datepicker__input-container input");
    if (jQuerydatepickerInput.val()) {
        datepickerInput = true;
    }

    // todo: check price filter - need a label class like 'checked-price-filter'
    // const jQueryinputPriceRadio = jQuery(".form-check-input[type=radio]");
    // if (jQueryinputPriceRadio.is(":checked")) {

...
class Phonehours extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
      ...
      if (osmOh) {
        ...
        if (outputArr.length === 0) {
          ...
          for (let i = 0; i < sortedDays.length; i++) {
            let cnt = 0;
            let dayoutput = [<div key={(i + 1)} className={"phone-hours-day"}>{weekdays[sortedDays[i]]}</div>];
            let intervals = [];
            for (let j = 0; j < outputArr.length; j++) {
              if (outputArr[j][0] === sortedDays[i]) {
                let day = <div key={(j + 2) * 100} className={"phone-hours-day-interval"}>{outputArr[j][1]}</div>;
                intervals.push(day);
                cnt++;
              }
            }
            ...
            if (cnt === 0) {
              let day = <div key={(i + 1) + 300} className={"phone-hours-day-interval"}>Nicht erreichbar</div>;
              intervals.push(day);
            }

            days.push(<div className={"phone-hours-day-entry"} key={(i + 3) * 1000}>
              {dayoutput}
              <div className={"phone-hours-day-interval-wrapper"}>{intervals}</div>
            </div>);
          }
          daysNode = <div key={1} className={"phone-hours-days"}>{days}</div>;
        }
      }
    }
    if ((osmStringReadable !== null) && (osmStringReadable !== '')) {
      osmNode = <div key={2} className={"phone-hours-opened-state"}>{osmStringReadable}</div>;
    }
    if (hint !== null) {
      hintNode = <div key={3} className={"phone-hours-opened-hint"}>{hint}</div>;
    ...
    if (this.alwaysOpenStrings.includes(osmString)) {
      daysNode = this.props.languageRefs.ALWAYS_REACHABLE;
      osmStringReadable = "";
    }

    if (osmNode || hintNode) {
      openingHours = <div className={"phone-hours-opened"}>{osmNode}{hintNode}</div>;
    }
  ...
  getReadableState(startString, endString, oh, osmString) {
    ...
    if (oh.getUnknown()) {
      ...
      output += this.props.languageRefs.MAYBE_REACHABLE
        + (comment ? this.props.languageRefs.DEPENDING_UPON + comment + '"' : '');
    } else {
      output += ' ' + (oh.getState() ? this.props.languageRefs.CURRENTLY_REACHABLE : this.props.languageRefs.CURRENTLY_UNREACHABLE)
